...
export class MusicPlayerService {
    ...
    public load_playlist(playlist: Song_Playlist, keep_hsitroy?: boolean): void {
    ...
    async preload_next_track(track_key: string): Promise<void> {
    ...
    private async update_media_session(song: Song_Data): Promise<void> {
        if ('mediaSession' in navigator && navigator.mediaSession && song) {
            navigator.mediaSession.metadata = new MediaMetadata({
                ...
                // album: song.album,
                artwork: [
                    { src: await this.media.get_song_artwork(song) || "", sizes: '512x512', type: 'image/png' }
                ]
            });
        }
    }
    ...
    async skip_to_next(): Promise<void> {
        ...
        if(this.current_playlist) {
            this.load_playlist(this.current_playlist, true); // Keep history
        } else {
